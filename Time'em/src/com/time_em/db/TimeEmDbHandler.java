...
public class TimeEmDbHandler extends SQLiteOpenHelper {
		...
		for (int i = 0; i < taskList.size(); i++) {
			...
			try {
				...
				if (cursor.moveToFirst()) {
					// updating row
					int a = db.update(TABLE_TASK, values, Id + " = ?",
							new String[] { String.valueOf(taskEntry.getId()) });
				} else {
	...
	public void updateTeam(ArrayList<User> team) {
		...
		for (int i = 0; i < team.size(); i++) {
			...
			try {
				...
				if (cursor.moveToFirst()) {
					// updating row
					int a = db.update(TABLE_TEAM, values, UserId + " = ?",
	...
	public ArrayList<TaskEntry> getTaskEnteries(String userId) {
